...
5. **真实运行逻辑驱动的 Layout Studio**：本地设计工具复用项目 Scene 与 Adapter 语义，并把形态差异保存回受控的布局配置，而不是维护第二套预览算法。
6. **数据来源可追踪**：健康与运动心率保留来源语义；没有官方样本时使用等待状态，不把模拟值写成真实结果。
7. **可穿戴生命周期治理**：健康订阅、定位、计时器、动作传感器等资源随页面和运动状态启停，减少后台资源泄漏。
8. **面向约束的质量门禁**：除 lint 外，还覆盖 Scene、架构、交互、设计视图、适配器、健康数据、运动体验、持久化、功耗、触觉、文本适配和页面 bundle 预算。
