...
- `src/runtime/device_profile.js` 是物理设备事实的验证边界；它要求 Vela 提供 canonical `screenShape`，Scene 只负责设计坐标投影。
...
- Node.js 18+
- npm
- Vela JS 应用 Framework API Level 3+
...
3. Device Profile 只接受系统提供的 canonical `screenShape`，不得用宽高比猜设备形态。
4. 不通过组件宽度重新计算 safe area。
5. 不在 Adapter/Resolver/UX 中做 circle chord fitting、Y 扫描、自动缩放或运行时几何修复。
6. 页面 CSS 不拥有产品非零几何；几何必须来自 resolved Recipe。
7. 页面在 Recipe plan 就绪前不渲染产品 geometry。
8. Full-bleed scene 与 safe content 分离。
9. Product Math Engine 必须由 resolved Recipe/Plan 配置，不得成为第二套静态视觉 owner 或适配 solver。
10. Feature / Domain / Capability 逻辑不得因为视觉迁移重新塞回页面。
11. 健康和运动正式表面不得伪造系统健康数据；未知值保持 `null`/unavailable 直到 View 显示为 `--`。
12. 一个值只在其 owner 边界规范化一次；不要在 Capability、Domain、Feature、View 连续 Number/clamp/normalize。
13. 旧持久化 schema 不通过长期兼容代码修补；破坏性 V3 迁移使用干净 namespace，历史实现由 Git 保存。
14. 未来工具或模板不得成为第二套 Recipe/Adapter/Device Profile 规范。
