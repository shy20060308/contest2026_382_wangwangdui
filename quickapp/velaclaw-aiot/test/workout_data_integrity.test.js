...
const finalizedPolicy = require('../src/domain/workout/finalized_commit_policy')
...
assert.deepStrictEqual(finalizedPolicy.decide(null, { status: 'missing', blocked: false }, frozenActive), { action: 'commit', reason: 'active-missing' }, 'F02: missing active on finalized retry must not be recreated after an ambiguous delete')
const oldPersistedActive = Object.assign({}, frozenActive, { finishedAt: null })
assert.deepStrictEqual(finalizedPolicy.decide(oldPersistedActive, { status: 'ok', blocked: false }, frozenActive), { action: 'persist', reason: 'intent-not-durable' }, 'F02: an in-memory freeze with only an unfinalized durable active must persist the frozen intent before the record')
assert.deepStrictEqual(finalizedPolicy.decide(frozenActive, { status: 'ok', blocked: false }, frozenActive), { action: 'commit', reason: 'intent-durable' }, 'F02: a matching durable finalized intent resumes directly from record commit')
assert.strictEqual(finalizedPolicy.decide(null, { status: 'io-error', blocked: true }, frozenActive).action, 'block', 'F02: I/O uncertainty must not be treated as missing persistence')
assert.strictEqual(finalizedPolicy.decide(null, { status: 'corrupt', blocked: true }, frozenActive).action, 'block', 'F02: corrupt persistence must not advance completion')
assert.strictEqual(finalizedPolicy.decide(Object.assign({}, frozenActive, { id: 'other' }), { status: 'ok', blocked: false }, frozenActive).action, 'corrupt', 'F02: a different persisted session id is a conflict')
assert.strictEqual(finalizedPolicy.decide(Object.assign({}, frozenActive, { finishedAt: 62000 }), { status: 'ok', blocked: false }, frozenActive).action, 'corrupt', 'F02: a different persisted completion timestamp is a conflict')
...
assert.ok(controllerSource.includes('var decision = finalizedPolicy.decide(stored, persistence, finalized)'), 'F02: retries must distinguish durable intent, missing cleanup, blocked persistence and conflicts')
assert.ok(controllerSource.includes("if (decision.action === 'persist')"), 'F02: an in-memory frozen session whose durable active is still unfinalized must persist the intent first')
assert.ok(controllerSource.includes("if (decision.action === 'block') return"), 'F02: corrupt/I/O persistence must not be mistaken for an absent active key')
assert.ok(controllerSource.includes('if (!isFinalized(restored)) persist()'), 'F02: loading a finalized durable intent must not rewrite it during page startup')
...
console.log('Workout truthful-data contracts verified: finalized retry policy distinguishes frozen memory from durable persistence across crash windows')
