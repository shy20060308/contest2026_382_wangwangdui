...
assert.ok(finalizeIndex >= 0 && saveIndex > finalizeIndex && clearIndex > saveIndex, 'F02: finalized active intent must persist before completed record, and record before active deletion')
assert.ok(controllerSource.includes('if (!persisted(finalizeResult)) return'), 'finalized intent persistence failure must stop completion before record write')
...
console.log('Workout truthful-data contracts verified: no synthetic metrics and completion retries are stable across persistence crash windows')
