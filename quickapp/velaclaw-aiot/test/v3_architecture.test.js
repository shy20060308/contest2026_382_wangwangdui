...
routes.forEach(function (route) {
  ...
  ids.add(surface.id)
  assert.ok(/surface_(?:host(?:_stage)?|entry_stage)\.ux/.test(source), pageFile + ' must render an audited generic Surface Host composition')
  if (route === 'pages/clock') assert.ok(source.includes('surface_entry_stage.ux'), 'Clock must use the visible boot boundary around its isolated Stage host')
