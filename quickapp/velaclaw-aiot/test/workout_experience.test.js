...
assert.ok(registry.includes("status: finalized ? 'finalizing' : session.status"), 'Finalized persistence retry must have an explicit semantic presentation state')
assert.ok(registry.includes("if (!current || isFinalized()) return"), 'Finalized sessions must not expose pause/resume behavior')
assert.ok(registry.includes("if (isFinalized()) { finishAndNavigate(); return }"), 'Retry-save action must skip a second finish confirmation for an already-finalized session')
