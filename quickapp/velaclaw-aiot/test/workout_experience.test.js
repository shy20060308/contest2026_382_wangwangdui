...
assert.strictEqual(running.flowButtons.filter(item => item.id === 'finish')[0].copy.title, '结束')
...
assert.strictEqual(paused.flowButtons.filter(item => item.id === 'finish')[0].copy.title, '结束')
assert.strictEqual(paused.flowButtons.filter(item => item.id === 'gps')[0].copy.title, 'GPS 已暂停')

const finalizing = project({
  confirming: false,
  finalized: true,
  type: 'walk',
  status: 'finalizing',
...
assert.strictEqual(finalizing.flowHeaders[0].subtitleTrailing, '待保存')
assert.strictEqual(finalizing.flowButtons.filter(item => item.id === 'pause')[0].copy.title, '已结束 · 待保存')
assert.strictEqual(finalizing.flowButtons.filter(item => item.id === 'finish')[0].copy.title, '重试保存')
assert.strictEqual(finalizing.flowButtons.filter(item => item.id === 'finish')[0].action, 'workout-request-finish')
...
const binding = read('src/product/controller_bindings/workout.js')
...
assert.ok(controller.includes('if (!restored)'), 'Invalid persisted workout state must be rejected rather than repaired')
...
assert.ok(binding.includes("status: finalized ? 'finalizing' : session.status"), 'Finalized persistence retry must have an explicit semantic presentation state')
assert.ok(binding.includes("if (!current || isFinalized()) return"), 'Finalized sessions must not expose pause/resume behavior')
assert.ok(binding.includes("if (isFinalized()) { finishAndNavigate(); return }"), 'Retry-save action must skip a second finish confirmation for an already-finalized session')
...
assert.ok(page.includes("import controllerBinding from '../../product/controller_bindings/workout'"), 'Workout UX must statically import only its own controller binding')
assert.ok(page.includes('surfacePage.bind(this, surface, controllerBinding)'), 'Workout UX must bind the page-local Surface and controller through the generic runtime')
...
assert.ok(surface.includes('"running": { "text": "运动中"') && surface.includes('"paused": { "text": "已暂停"') && surface.includes('"finalizing": { "text": "待保存"'), 'Workout status copy and colors must be JSON-owned')
assert.ok(surface.includes('重试保存'), 'Finalized persistence retry must have an explicit JSON-owned action label')
...
console.log('Workout experience verified: truthful data, official heart rate, explicit finalized retry and page-local JSON presentation')
