...
assert.ok(page.includes("import controllerBinding from '../../product/controller_bindings/health'"), 'Health page must statically import only its own controller binding')
assert.ok(page.includes('surfacePage.bind(this, surface, controllerBinding)'), 'Health page must bind the Surface and its page-local controller exactly once through the generic runtime')
