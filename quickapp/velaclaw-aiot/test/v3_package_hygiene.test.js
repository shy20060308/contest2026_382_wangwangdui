...
const generatedRegistryFile = path.join(generatedRoot, 'surfaces.js')
...
const previewFile = path.join(generatedRoot, 'watchface_previews.js')
assert.ok(fs.existsSync(previewFile), 'Clock-derived watchface preview data must exist before AIoT staging')
const previewSource = fs.readFileSync(previewFile, 'utf8')
assert.ok(previewSource.startsWith('// AUTO-GENERATED by scripts/compile-v3-surfaces.js.'), 'watchface preview data must remain compiler-owned')
assert.strictEqual((previewSource.match(/\brequire\s*\(/g) || []).length, 0, 'page-local preview data must not re-import Clock or another Surface at runtime')
assert.ok(Buffer.byteLength(previewSource, 'utf8') < 120000, 'page-local watchface preview data must remain bounded rather than embedding unrestricted presentation history')

const pageFiles = filesUnder(path.join(sourceRoot, 'pages'), []).filter(function (file) { return /\.ux$/.test(file) })
const previewConsumers = pageFiles.filter(function (file) { return fs.readFileSync(file, 'utf8').includes('frontend/generated/watchface_previews') })
assert.deepStrictEqual(previewConsumers.map(function (file) { return path.relative(sourceRoot, file).split(path.sep).join('/') }), ['pages/watchface/index.ux'], 'generated watchface preview data must remain page-local')
