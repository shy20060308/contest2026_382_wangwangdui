...
const summaryCore = read('src/product/features/history/summary_core.js')
...
assert.ok(repository.includes("HISTORY_KEY = 'activity_history_v4'"), 'Truthful History must use the V4 persistence namespace')
assert.ok(!repository.includes("'activity_history_v3'"), 'Truthful History must not reopen synthetic V3 activity history')
...
assert.ok(!repository.includes("'health_history_7d'"), 'V4 must not reopen the persistence namespace that may contain seeded legacy history')
...
assert.ok(repository.includes('V4 history persistence must be an array'), 'Malformed V4 history containers must fail visibly')
assert.ok(repository.includes('Invalid V4 history field:'), 'Malformed V4 history fields must fail visibly')
...
assert.ok(controller.includes("require('./summary_core')"), 'History controller must delegate summary semantics to the executable core')
assert.ok(controller.includes("dayWindow.dateKey(new Date())"), 'History controller must provide an explicit today key instead of assuming the last record is today')
assert.ok(summaryCore.includes('heartCount ? Math.round(totalHeart / heartCount) : null'), 'History summary must preserve unavailable aggregate heart rate as null')
assert.ok(summaryCore.includes('item.date === todayKey'), 'History summary must match today by calendar date')
assert.ok(!summaryCore.includes('source[source.length - 1]'), 'History summary must not relabel the final stored record as today')
assert.ok(!summaryCore.includes('Number(item.steps)') && !summaryCore.includes('Number(item.avgHeartRate)'), 'History summary must trust Repository records')
...
assert.ok(page.includes("import controllerBinding from '../../product/controller_bindings/history'"), 'History page must statically import only its own controller binding')
assert.ok(page.includes('surfacePage.bind(this, surface, controllerBinding)'), 'History page must bind the page-local Surface and controller through the generic runtime')
...
console.log('History truth contracts verified: strict truthful persistence, calendar-aware summary semantics and one page-local JSON presentation authority')
