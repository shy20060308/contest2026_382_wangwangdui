...
assert.ok(!healthChannel.includes('fallbackTimer') && !healthChannel.includes('fallbackValue'), 'health gateway must not fabricate unavailable measurements')
...
assert.ok(motion.includes('requireInterval(options && options.interval)'), 'motion gateway must require an explicit sampling interval')
assert.ok(!motion.includes("options.interval ? options.interval : 'normal'"), 'motion gateway must not silently invent a sampling interval')
