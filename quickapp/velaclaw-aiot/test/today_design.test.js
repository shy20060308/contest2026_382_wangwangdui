...
assert.ok(!page.includes('<stack class="today-page') || !page.includes('<stack class="today-page {{ viewportClass }}" style="position: {{ viewportPosition }}; left: {{ viewportLeft }}; top: {{ viewportTop }}; width: {{ viewportWidth }}; height: {{ viewportHeight }}; opacity: {{ pageOpacity }}; margin-left: {{ pageOffset }}px;" @swipe='), 'pill must not inherit a root swipe recognizer')
...
assert.ok(page.includes('@touchstart="handlePillTouchStart"'), 'pill surfaces must own touch-start gesture sampling')
assert.ok(page.includes('@touchend="handlePillTouchEnd"'), 'pill surfaces must resolve gestures from touch end')
assert.ok(!page.includes('@touchmove="handlePillTouchMove"'), 'pill must not rely on browser-like touchmove cancellation to prevent scrolling')
assert.ok(!page.includes('preventDefault') && !page.includes('stopPropagation'), 'Vela gesture handling must not depend on DOM event cancellation semantics')
...
console.log('Today L2 verified: pill stays inside visible viewport while circle/rect retain surface gestures')
