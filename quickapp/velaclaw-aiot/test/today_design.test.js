...
assert.ok(page.includes('margin-left: {{ pageOffset }}px;" @swipe="handleSwipe"'), 'Today must register one page-level Vela swipe recognizer after viewport overflow is removed')
...
assert.ok(page.includes('.pill-date-hero { width: 168px; height: 116px; margin-top: 52px;'), 'pill summary must start at the shared 168px capsule safe inset')
assert.ok(page.includes('.pill-month-hero { width: 168px; height: 82px; margin-top: 52px;'), 'pill month hero must start at the shared 168px capsule safe inset')
assert.ok(page.includes("event.direction === 'up'"), 'page-level swipe must support next-month navigation')
assert.ok(page.includes("event.direction === 'down'"), 'page-level swipe must support previous-month navigation')
assert.ok(!page.includes('handlePillSwipe'), 'Today must not keep a second pill-specific recognizer after coordinate overflow is fixed')
...
console.log('Today L2 verified: full design canvas, capsule-safe composition and page-level swipe are locked')
