...
function createStore(storage) {
  ...
  function flushPersist() {
    ...
    storage.set(KEY, snapshot, function (result) {
      ...
      var waiters = persistWaiters; persistWaiters = []; var value = clone()
      for (var i = 0; i < waiters.length; i++) waiters[i](value, result)
    })
  }
  function finishLoad(stored) {
    cached = mergeStored(stored); for (var key in pending) cached[key] = pending[key]; pending = {}; loaded = true; loading = false
    var waiters = loadWaiters; loadWaiters = []; var value = clone(); for (var i = 0; i < waiters.length; i++) waiters[i](value); flushPersist()
  }
  function startLoad() { if (loading || loaded) return; loading = true; storage.getJSON(KEY, finishLoad, null) }
  ...
  return {
    load: function (callback) { if (loaded && !loading) { if (callback) callback(clone()); return } if (callback) loadWaiters.push(callback); startLoad() },
    getSnapshot: clone,
