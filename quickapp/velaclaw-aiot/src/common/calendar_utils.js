// 公历月历与农历换算。使用本地静态年表，避免依赖部分 Vela 版本不支持的 Intl 农历。
var LUNAR_INFO = [
...
function toLunar(date) {
  ...
  var offset = Math.floor((Date.UTC(year, month, day) - Date.UTC(1900, 0, 31)) / DAY_MS)
  ...
  for (var lunarMonth = 1; lunarMonth <= 12; lunarMonth++) {
    var regularDays = lunarMonthDays(lunarYear, lunarMonth)
    if (offset < regularDays) {
      return { year: lunarYear, month: lunarMonth, day: offset + 1, isLeap: false }
    }
    offset -= regularDays
    if (leap === lunarMonth) {
      var extraDays = leapDays(lunarYear)
      if (offset < extraDays) {
        return { year: lunarYear, month: lunarMonth, day: offset + 1, isLeap: true }
      }
      offset -= extraDays
    }
  }
...
// Kept local: this module is CJS (required directly by node tests), while utils.js is ESM.
function pad2(value) {
...
function buildMonth(year, monthIndex, today) {
  ...
  var previousDate = new Date(viewYear, viewMonth, 0)
  var previousDays = previousDate.getDate()
  var cells = []
  ...
  for (var index = 0; index < 42; index++) {
    ...
    if (cellDay < 1) {
      inMonth = false
      cellMonth--
      cellDay = previousDays + cellDay
    } else if (cellDay > currentDays) {
      inMonth = false
      cellMonth++
      cellDay -= currentDays
    }
    ...
    cells.push({
      ...
      inMonth: inMonth,
      isToday: isToday,
      textColor: isToday ? '#FFFFFF' : (inMonth ? '#E5E5EA' : '#4D4D52'),
      backgroundColor: isToday ? '#FF375F' : 'transparent'
    })
  }
  return cells
}
