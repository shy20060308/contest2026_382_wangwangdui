...
export function createHealthController(onChange) {
  ...
  function emit() {
    ...
    var model = {
      ...
      heartSource: { live: heartAvailable, errorCode: Number(data.heartRateErrorCode) || 0, mode: data.heartRateSource || 'unavailable' },
      spo2Source: { live: spo2Available, errorCode: Number(data.spo2ErrorCode) || 0, mode: data.spo2Source || 'unavailable' },
      stressSource: { live: stressAvailable, errorCode: Number(data.stressErrorCode) || 0, mode: data.stressSource || 'unavailable' },
