...
function applySample(state, sample) {
  var previousState = state || createState(); var current = { x: number(sample && sample.x), y: number(sample && sample.y), z: number(sample && sample.z) }; var currentMagnitude = magnitude(current)
  var calibrationSamples = previousState.calibrationSamples || 0, baseline = number(previousState.baseline)
  if (calibrationSamples < 12) { baseline = (baseline * calibrationSamples + currentMagnitude) / (calibrationSamples + 1); calibrationSamples++ }
  var delta = vectorDelta(current, previousState.previous), gravityOffset = Math.abs(currentMagnitude - baseline), score = Math.max(delta, gravityOffset)
