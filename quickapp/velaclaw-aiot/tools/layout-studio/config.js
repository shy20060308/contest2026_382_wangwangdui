...
const APPS = {
  heart: {
    ...
    groups: [
      ...
      group('header', '页头', '标题与状态摘要区域。', [number('headerWidth', '页头宽度', { min: 60, max: 192 }), number('headerHeight', '页头高度', { min: 12, max: 80 }), number('titleSize', '标题字号', { min: 5, max: 40 }), number('subtitleSize', '副标题字号', { min: 4, max: 30 })]),
      group('hero', '心率主卡', '心率主卡的高度、间距与内边距。', [number('heroOuterHeight', '卡片总高度', { min: 30, max: 180 }), number('cardGap', '卡片间距', { min: 0, max: 40 }), number('cardRadius', '圆角', { min: 0, max: 60 }), number('cardPaddingX', '左右内边距', { min: 0, max: 30 }), number('cardPaddingY', '上下内边距', { min: 0, max: 30 }), number('valueSize', '主数字字号', { min: 8, max: 60 })]),
      group('mini', '血氧 / 压力卡', '两张小卡共用这些尺寸。', [number('miniOuterHeight', '小卡总高度', { min: 24, max: 160 }), number('miniValueSize', '小卡数字字号', { min: 6, max: 50 }), number('labelSize', '标签字号', { min: 4, max: 30 })]),
      group('detail', '趋势详情卡', '血氧与压力趋势卡共用的高度和图表参数。', [number('detailOuterHeight', '详情卡总高度', { min: 28, max: 220 }), number('chartHeight', '图表高度', { min: 6, max: 120 }), number('trendMinHeight', '柱形最小高度', { min: 2, max: 60 }), number('scrollPaddingBottom', '底部留白', { min: 0, max: 100 })])
    ]
  },
  history: {
